...
<script type="text/javascript">
	...
	function removeFileCheckBx(fileId) {
		...
		if (chkCount > 0) {
			$(".removeUpFileBtn").removeAttr("disabled"); // disabled를 제거
			$(".removeUpFileBtn").val(chkCount + "개 파일을 삭제합니다.");
		} else if (chkCount == 0) {
			$(".removeUpFileBtn").attr("disabled", true); // disabled를 추가
			$(".removeUpFileBtn").val("선택한 파일 삭제");
		}
	}
	...
	function countCheckBoxChecked() {
		...
		$(".fileCheck").each(function(i, item) {
			...
			if ($(item).is(":checked")) { // class가 filecheck인 요소를 돌며 checked상태에 따라 true, false 반환
				result++;
			}
		});
	...
	function removeFile() {
		...
		$(".fileCheck").each(function(i, item) {
			if ($(item).is(":checked")) { // 파일 삭제하려고 체크박스를 체크했다면
				...
				removeFileArr.push(tmp); // 배열에 pk를 저장
			}
		});
	...
	function cancelRemFiles() {
		// 파일삭제 취소버튼 클릭시 호출
		$.ajax({
			url : "/hboard/cancelRemFiles", // 데이터가 송수신될 서버의 주소
			type : "POST", // 통신 방식 (GET, POST, PUT, DELETE)
			dataType : "json", // 수신 받을 데이터 타입 (MIME TYPE)
			async : false, // 동기통신방식
			success : function(data) { // 통신이 성공하면 수행할 함수
	...
	function addRows(obj) {
		...
		let inputFileTag = `<tr><td colspan='2'><input type='file' class='form-control' id='newFile_\${rowCnt}' name="modifyNewFile"
		onchange='showPreview(this);' /></td>
		<td><input type='button' class='btn btn-info' value='파일저장취소' onclick='cancelAddFile(this);'></td></tr>`;
	...
	function showPreview(obj) {
		...
		if(imageType.indexOf(fileType) != -1) { // 이미지 파일이라면
			...
			let imgTag = `<div style='padding: 60px;'><img src='/resources/images/noimage.png' width='50px'/><span>\${fileName}</span></div>`;
			$(imgTag).insertAfter(obj);
		}
	}
								...
								<tbody>
												...
												<td><c:choose>
														...
														<c:when test="${file.thumbFileName == null }">
															<a href="/resources/boardUpFiles${file.newFileName }">
																<img src="/resources/images/noimage.png" />${file.newFileName }
															</a>
														</c:when>
													</c:choose></td>
									...
									<tr>
										<td colspan="3" style="text-align: center;"><img alt=""
											src="/resources/images/add.png" width="100px"
											onclick="addRows(this);"></td>
									</tr>
								</tbody>
							</table>
